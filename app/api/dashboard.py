...
@api.route('/dashboard1/<int:id>')
def one_real_time_status(id):
    start, end = cert_date_range()
    status = ThCertification.query.filter(ThCertification.companyCode==id).filter(ThCertification.dtCertificate.between(start, end)).order_by(ThCertification.dtCertificate.desc()).all()
    g_status = groupby(status, lambda s: s.dtCertificate.strftime('%Y-%m-%d %H:%M'))
    report = ThReport.query.filter(ThReport.companyCode==id).filter(ThReport.dtCreated.between(start, end)).order_by(ThReport.dtCreated.desc()).all()
    g_report = groupby(report, lambda s: s.dtCreated.strftime('%Y-%m-%d %H:%M'))
...
@api.route('/dashboard2/<int:id>')
def one_real_time_static(id):
    start, end = static_date_range()
    statics = TsCertReportCount.query.filter(TsCertReportCount.companyCode==id).filter(TsCertReportCount.registerDt.between(start, end)).order_by(TsCertReportCount.registerDt.desc()).all()
    g_statics = groupby(statics, lambda s: s.registerDt.strftime('%Y-%m-%d'))
